...
export function App() {
  ...
  const detail = useQuery({ queryKey: ['title', selected], queryFn: ({ signal }) => getJSON<EventItem>(`/api/v1/titles/${selected}`, signal), enabled: Boolean(selected) })
  ...
  return <div className="min-h-screen">
    ...
    <main>
      ...
      <section className="shell discovery">
        ...
        <div className="results"><div className="results-head"><div><span>WORLDWIDE PREMIERES</span><h2>{heading}</h2></div><label>Sort<select value={search.sort ?? 'date_asc'} onChange={(e) => setSearch({ sort:e.target.value })}><option value="date_asc">Date, then title</option><option value="date_desc">Latest first</option><option value="title_asc">Title A–Z</option><option value="confidence_desc">Confidence</option></select></label></div>
...
function VirtualList({ items, onOpen }: { items:EventItem[]; onOpen:(id:string)=>void }) {
